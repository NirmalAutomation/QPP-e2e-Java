...
public class IA_PCMHattestation extends Base {
    ...
    @Test
    public void ia_pcmhAttestation () throws InterruptedException {

        // IA Page displayed
        G_IAPage ia = new G_IAPage(driver);
        Log.info("IA Page displayed");

        // IA PCMH 'Yes' button selection
        Thread.sleep(1000);
        ia.attestIA_PCMH().click();
        Log.info("Click action on IA_PCMHattestation button");

        // Category Success!
        try {
            Assert.assertEquals(ia.getCategorySuccess().getText(), "Category Success!");
