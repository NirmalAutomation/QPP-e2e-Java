...
public class IAFilterCheckboxes extends Base {
    ...
    @Test
    public void iaFilterCheckbox() throws InterruptedException {
        ...
        // Check the CEHRT Eligible
        try {
            ia.getCEHRTEligible().click();
            Log.info("Check action on CEHRT Eligible");
        }
        catch (Exception f) {
            Log.error("CEHRT Eligible not found");
        }

        // Validate the Showing Activities
        try {
            Assert.assertEquals(ia.getActivitiesShowing().getText(), "Showing 1 Activities");
