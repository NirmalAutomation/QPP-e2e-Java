...
public class ACIHide_Show extends Base {
    ...
    @Test(priority = 0)
    public void aciHideButton() throws InterruptedException {

        // ACI Page displayed
        F_ACIPage ac = new F_ACIPage (driver);
        Log.info("ACI Page displayed");

        // Hide the ACI Information
        Thread.sleep(1000);
        ac.getHide().click();
        Log.info("Click action on ACI Hide Button");

        // Show the ACI Information
        Thread.sleep(2000);
        ac.getShow().click();
        Log.info("Click action on ACI Show Button");

    }

    @Test(priority = 1)
    public void assertACIInformation() throws InterruptedException {
        ...
        // Assert the EHR Text
        try {
            Assert.assertEquals(ac.getEhrOptionalText().getText(), "Certified EHR Technology Lookup Tool (Optional)");
            Log.info("ACI page has EHR (CHPL) Lookup");
        }
        catch (Exception e){
            Log.error("ACI page doesn't have EHR (CHPL) Lookup");
        }
